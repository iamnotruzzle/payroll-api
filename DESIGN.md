...
- **Quick app grid:** The navbar and sidebar All Apps controls open a flat, permission-filtered quick-launch grid without leaving the current task. `Alt+A` opens it from anywhere in the authenticated shell. It uses a translucent blue-black veil and one blurred paper panel inspired by a desktop application launcher, while preserving the product's institutional palette and written availability states. The account-menu All Apps link continues to open the full grouped workspace.
