...
class PayrollDeductionService
{
    ...
    public function save(array $data, int $id = 0): PayrollDeduction
    {
        $item = $id ? PayrollDeduction::findOrFail($id) : new PayrollDeduction();
        $item->fill($data);
        $item->save();
        return $item;
    }
    ...
    {
        return PayrollDeduction::findOrFail($id)->delete();
    }
}
