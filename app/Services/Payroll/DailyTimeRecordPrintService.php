...
class DailyTimeRecordPrintService
{
    /**
     * @return array{employee: Employee, period: CarbonImmutable, rows: Collection<int, array<string, mixed>>, regularHours: string, saturdayHours: string, verifierName: string, verifierDesignation: string}
     */
    public function buildPrintPayload(string $empId, int $month, int $year): array
    {
        ...
        $leaveDates = $this->leaveDates($employee->emp_id, $from, $to);
        ...
        $rows = collect(range(1, 31))->map(function (int $day) use ($period, $to, $dtrs, $adjustments, $labels, $labelOptions, $holidays, $leaveDates) {
            ...
            $timeinAm = $this->formatTime($dtr?->timein_am);
            $timeoutAm = $this->formatTime($dtr?->timeout_am);
            $timeinPm = $this->formatTime($dtr?->timein_pm);
            $timeoutPm = $dtr?->timeout_nextday
                ? CarbonImmutable::parse($dtr->timeout_nextday)->format('h:i A')
                : $this->formatTime($dtr?->timeout_pm);
            $hasPunches = filled($timeinAm) || filled($timeoutAm) || filled($timeinPm) || filled($timeoutPm);

            return [
                'day' => $day,
                'date' => $date,
                'weekday' => $dateValue->format('D'),
                'timein_am' => $timeinAm,
                ...
                'label' => ! $hasPunches ? $this->labelText($label, $labelOptions, $leave, $holiday, $weekendLabel) : '',
    ...
    public function pdfResponse(array $payload): Response
    {
        $pdf = new FPDF();
        $pdf->SetTitle('Daily Time Record', true);
        $this->appendEmployeePage($pdf, $payload);
    ...
    /**
     * @param  array<int, array{employee: Employee, period: CarbonImmutable, rows: Collection<int, array<string, mixed>>, regularHours: string, saturdayHours: string, verifierName: string, verifierDesignation: string}>  $payloads
     */
    public function pdfBulkResponse(array $payloads): Response
    {
        $pdf = new FPDF();
        $pdf->SetTitle('Daily Time Records', true);
        ...
        if ($payloads === []) {
            ...
            $pdf->Cell(0, 10, 'No employees found for bulk DTR print.', 0, 1, 'C');
        }
    ...
    private function appendEmployeePage(FPDF $pdf, array $payload): void
    {
        $pdf->AddPage();
        $this->drawCopy($pdf, 5, false, $payload);
        $this->drawCopy($pdf, 110, true, $payload);
    }
    ...
    private function labelText(?PayrollDtrLabel $label, Collection $labelOptions, ?array $leave, ?PayrollHoliday $holiday, ?string $weekendLabel): string
    {
        if ($holiday) {
        ...
        if ($label) {
            ...
            return trim((string) ($label->remarks ?: $fallback));
        }
     ...
     */
    private function leaveDates(string $employeeId, CarbonImmutable $from, CarbonImmutable $to): array
    {
        $dates = [];
    ...
    private function drawCopy(FPDF $pdf, float $xInitial, bool $rightCopy, array $data): void
    {
